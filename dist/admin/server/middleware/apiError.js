...
module.exports = function (req, res, next) {
	res.apiError = function apiError(statusCode, error, detail) {
		...
		if (!detail && typeof error === 'object' && error.toString() === '[object Object]' && error.error && error.detail) {
			detail = error.detail;
			error = error.error;
		}
		...
		// send error as json
		var data = typeof error === 'string' || error && detail ? { error: error, detail: detail } : error;
		res.json(data);
		return assign({
