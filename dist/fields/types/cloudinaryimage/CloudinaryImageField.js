...
module.exports = _Field2.default.create({
	...
	renderActionInput() {
		...
		if (this.state.userSelectedFile || this.state.removeExisting) {
			const value = this.state.userSelectedFile ? `upload:${this.state.uploadFieldPath}` : '';
	...
	renderUI() {
		...
		const imageContainer = _react2.default.createElement(
			'div',
		...
		const toolbar = this.shouldRenderField() ? this.renderImageToolbar() : _react2.default.createElement(_elemental.FormInput, { noedit: true });
